...
TEST(DemodulatorTest, TestFilterFunction)
{
    ...
    // проверка
    ASSERT_EQ(filter(b, a, numbers), required_answer);
}
...
TEST(DemodulatorTest, CheckGetComplexSignalFromBinaryFile)
{
    ...
    writeToDatFile("numbers.dat", numbers);
    ...
    // проверяем
    std::vector<std::complex<int>> functionReturned = getComplexSignalFromBinaryFile("numbers.dat");
...
TEST(DemodulatorTest, demodulateAMSignal)
{
    ...
    std::vector<float> demodulatedSignal = demodulateAMSignal(complexSamples, 0, 0);
...
TEST(DemodulatorTest, TestAverageFilter)
{
    ...
    ASSERT_EQ(averageFilter(numbers, windowSize), meanValues);
    // тестируем для другого набора чисел
    numbers                            = {2, 1, 6, 2, 4, 3};
    std::vector<float> required_answer = {0.6667, 1.0, 3.0, 3.0, 4.0, 3.0};
    ASSERT_EQ(averageFilter(numbers, windowSize), required_answer);
}
...
{
    std::vector<std::complex<int>> complexSamples = getComplexSignalFromBinaryFile("am_sound.dat");
    std::vector<float> demodulatedSignal          = demodulateAMSignal(complexSamples, -5, 3.2);
    std::vector<float> filteredSignal             = averageFilter(demodulatedSignal, 10);

    // std::vector<float> filteredSignalmatlab = readFloatDataFromBinaryFile("signal_after_average_filter.mat");
    // ASSERT_EQ(filteredSignal, filteredSignalmatlab);
    // writeToWavFile("demodulated_AM", filteredSignal);
    writeToWavFile("demodulated_AM", demodulatedSignal, 16000);
}
...
TEST(DemodulatorTest, testDownsampleFunc)
{
    ...
    ASSERT_EQ(downsample(complexSamples, 2), downsampledAnswer);
}
...
TEST(DemodulatorTest, testDemodulationFM)
{
    ...
    std::vector<float> demodulatedSignal = demodulateFMSignal(complexSamples);
